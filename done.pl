...
team(T) :- between(1, 15, T).
slot(S) :- between(1, 274, S).
...
at_most_2_games(R_max, T1) :-
    slot(S),
    S_end #= S + R_max - 1,
    findall(Slot, (scheduled(T1, _, Slot), between(S, S_end, Slot)), TeamSlots1),
    findall(Slot, (scheduled(_, T1, Slot), between(S, S_end, Slot)), TeamSlots2),
...
at_least_m_slots_apart(M, T1, T2) :-
    \+ (scheduled(T1, T2, Slot1), scheduled(T2, T1, Slot2), abs(Slot1 - Slot2) #< M).
...
print_schedule :-
    ...
    forall(member((T1, T2, S), SortedSchedule),
           format('Game between team ~w and team ~w at slot ~w~n', [T1, T2, S])).

% Run the scheduling and print the result
run(R_max, M) :-
